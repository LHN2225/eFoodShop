...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(value = "SELECT * FROM ORDER_1 WHERE SHIPPER_ID IS NULL AND SHIPPING_STATUS = ?2 AND ID = ?1 AND ORDER_1.IS_DELETED = 0", nativeQuery = true)
    Order findNotBusyOrderById(Long id, String shippingStatus);

    @Query(value = "SELECT * FROM ORDER_1 WHERE SHIPPER_ID IS NOT NULL AND SHIPPING_STATUS = ?2 AND ID = ?1 AND ORDER_1.IS_DELETED = 0", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM ORDER_1 WHERE SHIPPER_ID IS NULL " +
            "AND SHIPPING_STATUS = ?1 AND ORDER_1.IS_DELETED = 0 OFFSET (?2 - 1) * ?3 ROWS FETCH NEXT ?3 ROWS ONLY", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM ORDER_1 WHERE SHIPPER_ID = ?1 " +
            "AND SHIPPING_STATUS = ?2 AND ORDER_1.IS_DELETED = 0 OFFSET (?3 - 1) * ?4 ROWS FETCH NEXT ?4 ROWS ONLY", nativeQuery = true)
    List<Order> findByShipperIdAndShippingStatus(Long shipperId, String shippingStatus, int pageNumber, int pageSize);

    @Query(value = "SELECT * FROM ORDER_1 AND ORDER_1.IS_DELETED = 0 FETCH OFFSET (?1 - 1) * ?2 ROWS FETCH NEXT ?2 ROWS ONLY", nativeQuery = true)
